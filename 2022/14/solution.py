...
def main():
    ...
    for line in sys.stdin:
        coords = list(
            map(
                lambda item: Coord(
                    y=int(item.split(',')[1]), x=int(item.split(',')[0])
                ),
                line.strip().split(' -> '),
            )
        )
        for i in range(1, len(coords)):
            for x in range(
                min(coords[i - 1].x, coords[i].x), max(coords[i - 1].x, coords[i].x) + 1
            ):
                for y in range(
                    min(coords[i - 1].y, coords[i].y),
                    max(coords[i - 1].y, coords[i].y) + 1,
                ):
                    grid[y][x] = '#'
