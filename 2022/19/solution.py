...
class Inventory:
    ...
    def can_afford(self, costs: Self) -> bool:
        return (costs.ore <= self.ore and
                costs.clay <= self.clay and
                costs.obsidian <= self.obsidian and
                costs.geode <= self.geode)
...
@cache
def max_geodes(blueprint: Blueprint, inventory: Inventory, robots: Inventory, minutes: int, ignore: tuple = ()):
    if minutes == 0:
        return inventory.geode

    best = inventory.geode + robots.geode * minutes

    if (inventory.can_afford(blueprint.geode) and
            minutes > 1 and 'geode' not in ignore):
    ...
    if (inventory.can_afford(blueprint.obsidian) and
            robots.obsidian < blueprint.geode.obsidian and
            minutes > 2 and 'obsidian' not in ignore):
        best = max(best, max_geodes(
            blueprint=blueprint,
    ...
    if (inventory.can_afford(blueprint.clay) and
            robots.clay < blueprint.obsidian.clay and
            minutes > 2 and 'clay' not in ignore):
        best = max(best, max_geodes(
            blueprint=blueprint,
    ...
    if (inventory.can_afford(blueprint.ore) and
            robots.ore < blueprint.max_ore and
            minutes > 2 and 'ore' not in ignore):
        best = max(best, max_geodes(
            blueprint=blueprint,
    ...
    if inventory.ore <= blueprint.max_ore + 1 and minutes > 2:
        best = max(best, max_geodes(
            ...
            ignore=tuple([
                ...
                'obsidian' if inventory.can_afford(blueprint.obsidian) else None,
...
def main():
    ...
    print(sum(
        blueprint.id * max_geodes(blueprint, inventory=Inventory(), robots=Inventory(ore=1), minutes=24)
        for blueprint in blueprints
    ))

    print(prod(
        max_geodes(blueprint, inventory=Inventory(), robots=Inventory(ore=1), minutes=32)
        for blueprint in blueprints
        if blueprint.id <= 3
    ))
