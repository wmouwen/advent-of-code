...
def main():
    ...
    print(6 * len(droplet) - sum(2 for a, b in combinations(droplet, r=2) if a.is_neighbor(b)))
    ...
    while not queue.empty():
        ...
        for direction in neighbor_directions:
            neighbor = Vector(x=cube.x + direction.x, y=cube.y + direction.y, z=cube.z + direction.z)

            if not (lower.x <= neighbor.x <= upper.x and
                    lower.y <= neighbor.y <= upper.y and
                    lower.z <= neighbor.z <= upper.z):
                continue
