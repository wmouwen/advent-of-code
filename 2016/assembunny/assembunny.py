...
class AssembunnyComputer:
    def __init__(
        self, instructions: list[Instruction], output_callback: OutputCallback = None
    ):
    ...
    def _detect_addition(
        self, instructions: list[Instruction], offset: int
    ) -> DetectedFunction | None:
        ...
        if (
            ...
            and instructions[1][0] == 'dec'
            and instructions[2] == ['jnz', instructions[1][1], '-2']
        ):
    ...
    def _detect_multiplication(
        ...
        if (
            instructions[0][0] == 'cpy'
            and instructions[1][0] == 'inc'
            and instructions[2] == ['dec', instructions[0][2]]
            and instructions[3] == ['jnz', instructions[0][2], '-2']
            and instructions[4][0] == 'dec'
            and instructions[5] == ['jnz', instructions[4][1], '-5']
        ):
            return DetectedFunction(
                target=instructions[1][1],
                result=self._read_arg(instructions[0][1])
                * self.read(instructions[5][1]),
    ...
    def run(self) -> None:
        while not self._interrupted and 0 <= self._ip < len(self._instructions):
            if multiplication := self._detect_multiplication(
                self._instructions, self._ip
            ):
                self.write(
                    multiplication.target,
                    self.read(multiplication.target) + multiplication.result,
                )
                ...
                self.write(
                    addition.target, self.read(addition.target) + addition.result
                )
                ...
                match instruction:
                    ...
                    case 'tgl':
                        ...
                        if 0 <= target < len(self._instructions):
                            self._instructions[target][0] = TOGGLE_MAP[
                                self._instructions[target][0]
                            ]
