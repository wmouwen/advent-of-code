...
def read_input() -> Boss:
    ...
    for line in sys.stdin:
        match = re.match(r'(?P<type>Hit Points|Damage): (?P<quantity>\d+)', line.strip())
...
def apply_effects(player: Player,
                  boss: Boss,
    ...
    for active_effect in effects:
        ...
        if active_effect.turns > 1:
            remaining_effects.append(active_effect._replace(turns=active_effect.turns - 1))
...
def player_turn(player: Player,
    ...
    # Player turn
    if (player.mana >= 53
            and player.mana_used + 53 < best_so_far):
        score = player_attack(Effect.MAGIC_MISSILE, player, boss, hard_mode, effects, best_so_far)
        if score is not None and score < best_so_far:
            best_so_far = score

    if (player.mana >= 73
            and player.mana_used + 73 < best_so_far):
        score = player_attack(Effect.DRAIN, player, boss, hard_mode, effects, best_so_far)
        if score is not None and score < best_so_far:
            best_so_far = score
    ...
    if (player.mana >= 113
        ...
        score = player_attack(Effect.SHIELD, player, boss, hard_mode, effects, best_so_far)
        if score is not None and score < best_so_far:
            best_so_far = score
    ...
    if (player.mana >= 173
        ...
        score = player_attack(Effect.POISON, player, boss, hard_mode, effects, best_so_far)
        if score is not None and score < best_so_far:
            best_so_far = score
    ...
    if (player.mana >= 229
        ...
        score = player_attack(Effect.RECHARGE, player, boss, hard_mode, effects, best_so_far)
        if score is not None and score < best_so_far:
            best_so_far = score
...
def player_attack(attack: Effect,
                  player: Player,
                  boss: Boss,
                  hard_mode: bool,
                  effects: list[ActiveEffect] = [],
    ...
    if attack == Effect.MAGIC_MISSILE:
        ...
        player = player._replace(
            ...
            mana_used=player.mana_used + 113
        )
        ...
        player = player._replace(
            ...
            mana_used=player.mana_used + 173
        )
        ...
        player = player._replace(
            ...
            mana_used=player.mana_used + 229
        )
...
def boss_turn(player: Player,
    ...
    # Boss turn
    player = player._replace(hit_points=player.hit_points - max(boss.damage - shield, 1))
