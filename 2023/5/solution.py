...
def find_lowest_location(
    seed_ranges: list[SeedRange], category_maps: list[list[MapRange]]
) -> int:
    for category_map in category_maps:
        breakpoints = sorted(
            set(map(lambda map_range: map_range.start, category_map))
...
seed_ranges = [
    SeedRange(start=seeds[i], length=seeds[i + 1], offset=0)
